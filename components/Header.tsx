...
export const Header: React.FC<HeaderProps> = ({ language, setLanguage, onLoginClick, isDashboard }) => {
  ...
  const handleScroll = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    // If we are in dashboard, we need to reload or callback to switch view first
    // ideally handled by parent, but for now assuming simple scroll only works on landing
    if (isDashboard) {
        window.location.href = href; // Fallback for simple reloading to main page sections
        return;
    }
  ...
  const scrollToTop = () => {
    window.location.reload(); // Simple way to reset to landing home
  };
